...
<VideoEmbed
  ...
  caption="If you'd rather watch the install before doing it, this covers the same ground as the steps below."
/>
...
Once you're in a session, run this before anything else:

```
/terminal-setup
```

It rebinds <Kbd>Shift</Kbd>+<Kbd>Enter</Kbd> to insert a newline. Without it, pressing
Enter to start a second line submits your half-written prompt instead — the single most
common thing that makes a first session feel broken.

<Callout type="tip">
Two more worth knowing on day one: `/context` shows how much of the context window
you've used, and `/clear` resets the conversation between unrelated tasks.
</Callout>
